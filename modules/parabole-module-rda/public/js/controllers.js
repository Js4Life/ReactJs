	...
	$scope.goTo = function(obj, idx){
		...
		switch(obj)
		{
			...
			case Constant.GLOSSARY: $scope.viewTitle=Constant.GLOSSARY;
							...
							break;
			case Constant.MODEL_TAB: $scope.viewTitle=Constant.MODEL_TAB;
							$state.go('landing.model');
							break;
...
.controller('dashboardProfileCtrl', function($scope, $state, $stateParams, SharedService, MockService) {
    ...
    var items = MockService.TimelineData;

    /*angular.forEach(items._data, function(obj){
    ...
    $scope.onSelect = function (obj) {
        ...
        if(selObj.content == "Current Status" || selObj.content == "Initial Submission"){
        	SharedService.STATUS_VIEW = { title:selObj.content, statusID:selObj.id };
...
.controller('statusCtrl', function($scope, $state, $stateParams, SharedService, MockService) {
	$scope.initialize = function( ){
		$scope.heading = SharedService.STATUS_VIEW;
		$scope.tableData = MockService.dashboardTable;

		var layoutGraphData = { series: MockService.StatusData.series[SharedService.STATUS_VIEW.statusID], categories: MockService.StatusData.categories[SharedService.STATUS_VIEW.statusID] };
		...
		var chart = drawGraph('stack', layoutGraphData, cfgObj);
	}
	...
	$scope.onColumnClick = function (obj){
		...
		//$scope.levelNodes = layoutGraphData[$scope.level];
		$state.go("landing.schedule");
	}

	function drawGraph(type, data, cfgObj) {
		var chartData = {GraphType: type, data: data};
		var c = new GRAPH.PlotWrapper( $('.graphContainer') , cfgObj);
		c.draw(chartData);
		return c;
	}
...
.controller('scheduleCtrl', function($scope, $state, $stateParams, SharedService, MockService) {
	$scope.initialize = function(){
		...
		$scope.tableData = $scope.scheduleData[0].tableData;
	}
	...
	$scope.cfgObj = { 	Title:'Business Segments', 
	...
	$scope.changeMode = function(evt,idx){
		var element = $(evt.currentTarget);
		$('.box').removeClass('navActive');
		$('.box').eq(idx).addClass('navActive');

		$scope.tableData = MockService.ScheduleData[idx].tableData;
		...
		var chart = drawGraph('stack', layoutGraphData, $scope.cfgObj);

		function drawGraph(type, data, cfgObj) {
			var chartData = {GraphType: type, data: data};
			var c = new GRAPH.PlotWrapper( $('.graphContainer') , $scope.cfgObj);
			c.draw(chartData);
			return c;
		}
	}

	var layoutGraphData = {	'series': MockService.ScheduleData[0].graphData.series,	'categories': MockService.ScheduleData[0].graphData.categories};
...
.controller('gapDetailsCtrl', function($scope, $state, $stateParams, SharedService, MockService) {
	$scope.initialize = function(){
		$scope.gapData = MockService.GapData;
		$scope.heading = {title: SharedService.SCHEDULE_VIEW.scheduleID + ' Field Gaps (Detail)'};
	}
...
.controller('newsimulateCtrl',function($scope, $state, $http, $stateParams, SharedService, RiskAggregateService, MockService){
	...
	$scope.doNext = function(){
		SharedService.getImpactLevelForGivenNodes($scope.request).then(function(data){
			//SharedService.getPathBetweenTwoNodesFromImpactedVertices($scope.request).then(function(data){
			if(data){
...
.controller('newsimulategraphCtrl',function($scope, $state, $http, $stateParams, $timeout, SharedService, MockService, graphService){
	...
	.controller('modelCtrl', function($scope, $state, $compile, $timeout, SharedService, MockService){
		$scope.initialize = function(){
			...
			$scope.dropdownElements = [
				{"position": 1, name: 'AUTO LGD MODEL'}
			];
            $scope.item = $scope.dropdownElements[0];
            $scope.lineageRequestValue = $scope.item.name;

			SharedService.getJsonForAllLinege($scope.lineageRequestValue).then(function(data){
				if(data){
        ...
        $scope.clickNode = function(nodeId){
            if(nodeId){
                var node = $scope.viz.findNodeById(nodeId);
                // exploreConcept(node);
                SharedService.getJsonForAllLinegeAgainstNodeId($scope.lineageRequestValue, node).then(function(data){
                    if(data){
                        $scope.glossarySets = data.glossarySets;
                        //$scope.rules = data.rules;
                        //$scope.graphData = {nodes: data.graphData.vertices, edges: []};
		...
		$scope.onSubmitFile = function () {
			$scope.fileName= {"fileName": $scope.inputData.file.name };
			SharedService.getJsonForAllLinegewithAdaptiveLearning(_.extend($scope.inputData, $scope.fileName)).then(function(data){
				if(!data){
		...
		$scope.callGetJsonForAllLinege = function() {
            SharedService.getJsonForAllLinege($scope.lineageRequestValue).then(function(data){
                if(data){
                    $scope.glossarySets = data.glossarySets;
                    //$scope.rules = data.rules;
		...
		function exploreConcept(conceptNode){
			...
			SharedService.getLineageGraphByConceptId(conceptNode.id).then(function(data){
				if(data){
					...
					console.log($scope.selectedConceptNodeName);
				}
			});
...
.controller('margeAggCtrl', function($scope, $state, SharedService, MockService, graphService){
	...
	/*$scope.onSelect = function( edgeObj , fromNode , toNode ){
		var func = $scope.viz.isEdgeSelected(edgeObj.id) ? 'unSelectEdge' : 'selectEdge';
		if( func == 'selectEdge' ){
			$scope.currentSelEdge = edgeObj;
		}
		else{
			$scope.currentSelEdge = undefined;
		}
	}*/
	$scope.viewDefinition = function(){
		if($scope.currentSelNode && $scope.currentSelNode.extraInfo && $scope.currentSelNode.extraInfo.definition){
			$scope.dsViewerHead = $scope.currentSelNode.name + " : Definition";
			$scope.allColumnStr = $scope.currentSelNode.extraInfo.definition;
			$('#dsViewer').modal('show');
		}
