package com.parabole.cecl.platform.lineage.paraboleHyperGraph;

import com.parabole.cecl.platform.lineage.GlossarytoDBMapper.glossarytodb_map;
import com.parabole.cecl.platform.lineage.bankInfoReader.BankInfoReader;
import com.parabole.cecl.platform.lineage.businessRuleReader.RuleDef;
import com.parabole.cecl.platform.lineage.businessUCReader.UCDef;
import com.parabole.cecl.platform.lineage.paraboleGraph.DGraph;
...
public class HyperGraph {
		...
		public void addConceptNode(int node_id, String node_name, String node_desc){
			...
			EquiConceptLine_element pConceptElement = new EquiConceptLine_element(0, node_id, node_name, node_desc, 0, null, null, null);
		...
		public void AddGlossarySetInHG(List<BankInfoReader.bankglossary_element_map> glossary_list){
			...
			for(int i = 0; i < glossary_list.size() ; i++){
				...
				EquiConceptLine_element pConceptElement = new EquiConceptLine_element(glossary_list.get(i).getGlossary_Set(), glossary_list.get(i).getGlossary_ID(), glossary_list.get(i).getGlossary_Name(),
																					  glossary_list.get(i).getGlossary_Des(), glossary_list.get(i).getGlossary_ID(), null, null, null);
				//Add this element to the line
				EquiConcept_list[glossary_list.get(i).getConcept_ID()].add(pConceptElement);
			}
		}
		
		public void AddGlossaryOwnershipSetInHG(List<BankInfoReader.bankglossary_element_ownership_map> glossary_list){
			//Scan through the glossary_list and add layers as needed
			//Read the glossary list within the loop
			for(int i = 0; i < glossary_list.size() ; i++){
				System.out.println("glossary_list.get(i).getGlossary_Set() " + glossary_list.get(i).getGlossary_Set());
				...
				// Add the new glossary element to the latest constructed layer
				DGraph_list.get(constructed_layer).addNodewithOwnership(glossary_list.get(i).getGlossary_ID(), glossary_list.get(i).getGlossary_Name(),
														   glossary_list.get(i).getGlossary_Des(), glossary_list.get(i).getGenerator(),
														   glossary_list.get(i).getReviewer(), glossary_list.get(i).getApprover());
				//update the EquiConceptLine
				//Create an EquiConceptLine_element for the current concept node
				EquiConceptLine_element pConceptElement = new EquiConceptLine_element(glossary_list.get(i).getGlossary_Set(), glossary_list.get(i).getGlossary_ID(), glossary_list.get(i).getGlossary_Name(),
																					  glossary_list.get(i).getGlossary_Des(), glossary_list.get(i).getGlossaryConcept_ID(), glossary_list.get(i).getGenerator(),
														                              glossary_list.get(i).getReviewer(), glossary_list.get(i).getApprover());
																					  
				//Add this element to the line
				EquiConcept_list[glossary_list.get(i).getGlossaryConcept_ID()].add(pConceptElement);
			}
		}

		public void SetGlossaryDBMap (List<glossarytodb_map>  pGlsDB_Map) {
			//this.pGlossaryDB_Map = pGlsDB_Map;
			System.out.println("+++++++++++++++++ SetGlossaryDBMap START ++++++++++++++++++");
			
			for(glossarytodb_map	pGlstoDB_elem : pGlsDB_Map){
				pGlossaryDB_Map.add(pGlstoDB_elem);
			}
			System.out.println("+++++++++++++++++ SetGlossaryDBMap END ++++++++++++++++++");
		...
		public class EquiConceptLine_element {
			...
			public EquiConceptLine_element(int layer_ID, int concept_ID, String concept_NAME, String desc, int ref_node, String generator, String reviewer, String approver){
				this.layer_id = layer_ID;
				this.concept_id = concept_ID;
				this.concept_name = concept_NAME;
				this.Desc = desc;
				this.ref_concept_id = ref_node;
		...
		public Lineage	GenerateLineage (RuleDef rule) {
			...
			for(int i = 0 ; i < rule.getConcept_length() ; i++) {
				...
				for(int k = 1 ; k < EquiConcept_list[ConceptList[i]].size(); k++) {
					...
					pGlossary.Concept_ID = pGlossaryElem.ref_concept_id;					
		...
		public Lineage	GenerateLineagewithOwnership (UCDef uc) {
			Lineage 	pLineage = new Lineage();
			int[]		ConceptList;
			//Load parameters from rule to Lineage
			//USe UC And Rule interchange-ably
			pLineage.UC_ID = uc.getUC_ID();
			pLineage.UC_Name = uc.getUC_Name();
			pLineage.UC_Des = uc.getUC_def();
			ConceptList = uc.getConcept_ID();
			System.out.println("uc.getConcept_length " + uc.getConcept_length());
			for(int i = 0 ; i < uc.getConcept_length() ; i++) {
				Concept_Elem	pConcept = new Concept_Elem();
				pConcept.Concept_ID = ConceptList[i];
				pConcept.Concept_Name = EquiConcept_list[ConceptList[i]].get(0).concept_name;
				pConcept.Concept_Des = null;
				...
				for(int k = 1 ; k < EquiConcept_list[ConceptList[i]].size(); k++) {
					...
					pGlossary.Glossary_Des = pGlossaryElem.Desc;
					pGlossary.Concept_ID = pGlossaryElem.ref_concept_id;					
					pGlossary.Generator = pGlossaryElem.Generator;
					pGlossary.Reviewer = pGlossaryElem.Reviewer;
					pGlossary.Approver = pGlossaryElem.Approver;
		...
		public class Glossary_Elem {
			...
			public int getGlossary_ConceptID() {
				return Concept_ID;
			}
			public void setGlossary_ConceptID(int concept_id) {
			...
			public void setApprover_ID(String Approver) {
				Approver = Approver;
			}
