...
// =============================================================================
package com.parabole.cecl.platform.graphdb;
...
public class Coral extends GraphDb {
    ...
    public Coral() {
        final String graphDbUrl = AppUtils.getApplicationProperty(RdaAppConstants.INDUSTRY + ".coral.graphdb.url");
        final String graphDbUser = AppUtils.getApplicationProperty(RdaAppConstants.INDUSTRY + ".coral.graphdb.user");
        final String graphDbPassword = AppUtils.getApplicationProperty(RdaAppConstants.INDUSTRY + ".coral.graphdb.password");
        final Integer graphDbPoolMinSize = AppUtils.getApplicationPropertyAsInteger(RdaAppConstants.INDUSTRY + ".coral.graphdb.pool.min");
        final Integer graphDbPoolMaxSize = AppUtils.getApplicationPropertyAsInteger(RdaAppConstants.INDUSTRY + ".coral.graphdb.pool.max");
    ...
    public int generateId(final String counterKeyName) throws AppException {
        ...
        try {
            dbTx.begin();
            final OCommandSQL query = new OCommandSQL("UPDATE " + RdaAppConstants.RDA_COUNTER_TABLE + " INCREMENT ID_VALUE = 1 RETURN AFTER @this WHERE ID_NAME = '" + counterKeyName + "'");
    ...
    public Map<Integer, String> getConfigurationNames(final String userId, final String configurationType) throws AppException {
        ...
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT CFG_ID, CFG_NAME FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_USER = '" + userId + "' AND CFG_TYPE = '" + configurationType + "'");
    ...
    public String getConfigurationDetail(final Integer configurationId) throws AppException {
        ...
        final ODatabaseDocumentTx dbNoTx = getDocDBConnectionNoTx();
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT CFG_INFO FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_ID = " + configurationId);
            final List<ODocument> results = dbNoTx.command(query).execute();
            if (CollectionUtils.isNotEmpty(results)) {
    ...
    public List<Map<String, String>> getConfigurationByUserId(final String userId, final String configurationType) throws AppException {
        ...
        final ODatabaseDocumentTx dbNoTx = getDocDBConnectionNoTx();
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT CFG_NAME, CFG_INFO FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_USER = '" + userId + "' AND CFG_TYPE = '" + configurationType + "'");
            final List<ODocument> results = dbNoTx.command(query).execute();
            results.forEach((final ODocument result) -> {
    ...
    public List<Map<String, String>> getConfigurationInfoByNameOnly(final String configurationName) throws AppException {
        ...
        final ODatabaseDocumentTx dbNoTx = getDocDBConnectionNoTx();
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT CFG_NAME, CFG_INFO  FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_NAME = '" + configurationName + "'");
            final List<ODocument> results = dbNoTx.command(query).execute();
            results.forEach((final ODocument result) -> {
            	final Map<String, String> outputMap = new HashMap<String, String>();
    ...
    public List<Map<String, String>> getConfigurationByName(final String configurationName) throws AppException {
        ...
        final ODatabaseDocumentTx dbNoTx = getDocDBConnectionNoTx();
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT CFG_NAME, CFG_INFO  FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_NAME like '" + configurationName + "_%' ");
            final List<ODocument> results = dbNoTx.command(query).execute();
            results.forEach((final ODocument result) -> {
                final Map<String, String> outputMap = new HashMap<String, String>();
                final String configurationNameCollected = result.field("CFG_NAME");
    ...
    public List<Map<String, String>> getConfigurationByName(final String userId, final String configurationName, final boolean useLikePattern) throws AppException {
        ...
        try {
            ...
            if (useLikePattern) {
                sqlQuery = "SELECT CFG_NAME, CFG_INFO FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_USER = '" + userId + "' AND CFG_NAME like '" + configurationName + "-%'";
            } else {
                sqlQuery = "SELECT CFG_NAME, CFG_INFO FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_USER = '" + userId + "' AND CFG_NAME = '" + configurationName + "'";
            }
    ...
    public Integer saveConfiguration(final String userId, final String configurationType, final String configurationName, final String configurationDetails) throws AppException {
        final Integer configurationId = generateId(RdaAppConstants.RDA_USER_CONFIGS);
        ...
        save(RdaAppConstants.RDA_USER_CONFIGS, dataMap);
    ...
    public void deleteConfiguration(final Integer configurationId) throws AppException {
        ...
        executeUpdate("DELETE FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_ID = " + configurationId);
    }
    ...
    public void deleteConfigurationByName(final String userId, final String configurationName) throws AppException {
        ...
        executeUpdate("DELETE FROM " + RdaAppConstants.RDA_USER_CONFIGS + " WHERE CFG_USER = '" + userId + "' AND CFG_NAME = '" + configurationName + "'");
    }
    ...
    public String saveConceptTags(final String nodeId, final String nodeName, final Map<String, Boolean> tagDetails) throws AppException {
        ...
        save(RdaAppConstants.RDA_CONCEPT_TAGS, dataMap); 
        return nodeId;
    }
    ...
    public Map<Integer, String> getAllConceptByTag(final String tag) throws AppException {
        ...
        final ODatabaseDocumentTx dbNoTx = getDocDBConnectionNoTx();
        try {
            final OSQLSynchQuery<ODocument> query = new OSQLSynchQuery<ODocument>("SELECT NODE_ID, NODE_NAME FROM " + RdaAppConstants.RDA_CONCEPT_TAGS + "WHERE '" + tag + "' = '" + true + "'");
            final List<ODocument> results = dbNoTx.command(query).execute();
            results.forEach((final ODocument result) -> {
