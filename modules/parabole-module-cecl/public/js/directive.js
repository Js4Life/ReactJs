...
.directive('disableScroll', function ($parse, $compile) {
    ...
    .directive('vizGraph', function ($parse, $compile, SharedService) {
        return function (scope, element, attrs) {
            var cfgObj =scope[attrs.vizGraph];
            var ngModel = $parse(attrs.ngModel);
            var config = cfgObj || {
            ...
            if(config.hierarchical){
                config.options={physics: {hierarchicalRepulsion: {nodeDistance: 168}},hierarchicalLayout: {nodeSpacing: 132}};
            }
...
.directive('toolbox', function ($parse, $compile) {
    return function (scope, element, attrs) {
        ...
        var tmp = $('<div class="panel-body" style="max-height:' + panelMaxHeight +';overflow:auto;"><div class="row" ng-repeat="tool in ' 
            + cfgArr.tools 
    ...
    return function (scope, element, attrs) {
        ...
        reader.onload = function (e) {
            scope.$apply(function (scope) {  
                ...
                ngModel.assign(scope.inputData.files, obj);
...
.directive('readMore', function() {
    return {
        ...
        transclude: true,
        replace: true,
        template: '<div style="min-height: 55px;max-width:75%;"></div>',
        scope: {
            moreText: '@',
            lessText: '@',
            words: '@',
